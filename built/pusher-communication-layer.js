 ...
 */
function isString(s) { return _.isString(s); }
/**
 ...
 */
function allStrings(arr) { return _.every(arr, isString); }
/**
...
class PusherCommunicationLayer {
    ...
    constructor(authInfo) {
        ...
        this.emitTimeout = false; // The ID of the next send timeout
        this.channelQueue = []; // A queue of channel names to send messages on
     ...
     */
    removeFromChannelQueue(channelName, count) {
    ...
    shiftMessageFromQueue() {
        ...
        if (this.emitTimeout === false) {
            ...
            if (this.channelQueue.length > 0) {
                const channelName = this.channelQueue[0];
                const messageQueue = this.messageQueues[channelName];
                let nextItem = messageQueue.shift();
                const arrayToSend = [nextItem]; // the list of payloads we're going to send
