...
export class ChatCodesSocketIOClient {
	...
	constructor(private host:string, private port:number) {
		this.manager = new Promise((resolve, reject) => {
			resolve(new Manager(`http://${this.host}:${this.port}`));
		});
	}
	private getNamespace(name:string):Promise<SocketIOClient.Socket> {
		if(_.has(this.namespaces, name)) {
			...
			this.namespaces[name] = this.manager.then((manager) => {
				return manager.socket(`/${name}`);
			});
			return this.namespaces[name];
		}
	};
